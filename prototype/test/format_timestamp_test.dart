...
void main() {
  group('formatTimestamp', () {
    ...
    test(
        ...
        () {
      final timestamp = fixedNow.subtract(Duration(days: 3));
      expect(formatTimestamp(timestamp, now: fixedNow),
          equals(DateFormat('E').format(timestamp)));
    });

    test('returns correct day and month for timestamps earlier this year', () {
      final timestamp =
          DateTime(fixedNow.year, fixedNow.month - 1, fixedNow.day);
      expect(formatTimestamp(timestamp, now: fixedNow),
    ...
    test('returns day, month and year for timestamps from previous years', () {
      final timestamp =
          DateTime(fixedNow.year - 1, fixedNow.month, fixedNow.day);
      expect(formatTimestamp(timestamp, now: fixedNow),
    ...
    test('handles timestamps exactly at the start of today', () {
      final timestamp =
          DateTime(fixedNow.year, fixedNow.month, fixedNow.day, 0, 0, 0);
