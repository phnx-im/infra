...
fn backend_interaction() {
    ...
    let (credential_csr, signing_key) =
        ClientCredentialCsr::new(client_id.clone(), SignatureScheme::ED25519).unwrap();
    ...
    let mut user_record = UserRecord::new(client_id.clone(), encrypted_user_profile.clone());
    ...
    let new_user_profile_key = UserProfileKey::random(&client_id).unwrap();
